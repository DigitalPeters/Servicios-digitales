...
async function initDatabase() {
  ...
  // Renovaciones: permite controlar por producto si vendedores/distribuidores pueden renovar.
  await pool.query(`ALTER TABLE products ADD COLUMN IF NOT EXISTS renewable INTEGER DEFAULT 0`);
  await pool.query(`ALTER TABLE products ADD COLUMN IF NOT EXISTS renewal_days INTEGER DEFAULT 30`);
  await pool.query(`UPDATE products SET renewable=0 WHERE renewable IS NULL`);
  ...
  await pool.query(`ALTER TABLE account_reports ADD COLUMN IF NOT EXISTS provider_outcome TEXT DEFAULT ''`);
  await pool.query(`ALTER TABLE account_reports ADD COLUMN IF NOT EXISTS provider_outcome_note TEXT DEFAULT ''`);
  await pool.query(`ALTER TABLE account_reports ADD COLUMN IF NOT EXISTS provider_refund_amount NUMERIC DEFAULT 0`);
  await pool.query(`ALTER TABLE account_reports ADD COLUMN IF NOT EXISTS provider_replacement_details TEXT DEFAULT ''`);
...
app.post("/api/admin/create-product", authMiddleware, adminMiddleware, async (req, res) => {
  try {
    const { name, description, price, cost_price, category, required_fields, charge_mode, stock_enabled, stock, product_type, combo_items, combo_discount, renewable, renewal_days } = req.body;
    ...
    await pool.query(
      `INSERT INTO products
       (name, description, price, cost_price, category, required_fields, charge_mode, active, stock_enabled, stock, product_type, combo_items, combo_discount, owner_admin_id, renewable, renewal_days)
      ...
      [
        ...
        (renewable === true || renewable === 1 || renewable === '1' || renewable === 'true') ? 1 : 0,
        Math.max(1, Math.floor(Number(renewal_days || 30)))
      ]
    );
  ...
  try {
    ...
    const totalsResult = await pool.query(
      `SELECT COUNT(*)::int AS total,
              ...
              COUNT(*) FILTER (WHERE account_reports.provider_reported_at IS NOT NULL)::int AS provider_reports_total,
              COUNT(*) FILTER (WHERE account_reports.provider_responded_at IS NOT NULL)::int AS provider_responses_total,
              COUNT(*) FILTER (WHERE account_reports.provider_reported_at IS NOT NULL AND account_reports.provider_responded_at IS NULL)::int AS provider_pending_total,
              COUNT(*) FILTER (WHERE lower(COALESCE(account_reports.provider_outcome,'')) = 'cuenta_reemplazo')::int AS provider_replacements_total,
              COUNT(*) FILTER (WHERE lower(COALESCE(account_reports.provider_outcome,'')) = 'reembolso')::int AS provider_refunds_total,
              COUNT(*) FILTER (WHERE lower(COALESCE(account_reports.provider_outcome,'')) = 'sin_solucion')::int AS provider_no_solution_total
       FROM account_reports
       WHERE ${scopeSql}`,
      [ownerId]
    );
    ...
    const providerSummary = {
      reports: Number(totalsResult.rows[0]?.provider_reports_total || 0),
      responses: Number(totalsResult.rows[0]?.provider_responses_total || 0),
      pending: Number(totalsResult.rows[0]?.provider_pending_total || 0),
      replacements: Number(totalsResult.rows[0]?.provider_replacements_total || 0),
      refunds: Number(totalsResult.rows[0]?.provider_refunds_total || 0),
      no_solution: Number(totalsResult.rows[0]?.provider_no_solution_total || 0)
    };
    ...
    const result = await pool.query(
      `SELECT
        ...
        account_reports.provider_outcome,
        account_reports.provider_outcome_note,
        account_reports.provider_refund_amount,
        account_reports.provider_replacement_details,
        account_reports.provider_response_updated_at,
    ...
    res.json(buildPaginationPayload(result.rows, page, limit, total, { pendingTotal, providerSummary }));
  } catch (err) {
    console.error(err.message);
    res.status(500).json({ error: "Error cargando reportes de cuenta" });
  }
...
// ADMIN: REGISTRAR / EDITAR RESPUESTA DEL PROVEEDOR
app.patch("/api/admin/account-reports/:reportId/provider-response", authMiddleware, adminMiddleware, async (req, res) => {
  try {
    ...
    const outcome = String(req.body?.provider_outcome || '').trim().toLowerCase();
    const outcomeNote = String(req.body?.provider_outcome_note || '').trim().slice(0, 2000);
    const replacementDetails = String(req.body?.provider_replacement_details || '').trim().slice(0, 1000);
    const refundAmount = Math.max(0, Number(req.body?.provider_refund_amount || 0));

    const validOutcomes = ['', 'cuenta_reemplazo', 'reembolso', 'sin_solucion', 'otro'];
    if (!validOutcomes.includes(outcome)) return res.status(400).json({ error: 'Resultado del proveedor inválido' });
    if (!Number.isFinite(refundAmount)) return res.status(400).json({ error: 'Monto de reembolso inválido' });
    ...
    const q = await pool.query(`
      ...
      SET provider_responded_at = COALESCE($1::timestamptz, ar.provider_responded_at, NOW()),
          provider_response = $2,
          provider_outcome = $3,
          provider_outcome_note = $4,
          provider_refund_amount = $5,
          provider_replacement_details = $6,
          provider_response_updated_at = NOW(),
          reviewed_at = COALESCE(ar.reviewed_at, NOW())
      WHERE ar.id=$7
        AND ($8::int IS NULL OR ar.owner_admin_id=$8 OR ar.user_id=$8 OR ar.user_id IN (SELECT id FROM users WHERE owner_user_id=$8)
             OR (ar.direct_customer_report=TRUE AND EXISTS (SELECT 1 FROM orders o WHERE o.id=ar.order_id AND o.admin_quick_sale=TRUE AND o.owner_admin_id=$8)))
      RETURNING id, provider_reported_at, provider_responded_at, provider_response,
                provider_outcome, provider_outcome_note, provider_refund_amount,
                provider_replacement_details, provider_response_updated_at`,
      [respondedAt, note, outcome, outcomeNote, refundAmount, replacementDetails, reportId, ownerId]);

    if (!q.rows[0]) return res.status(404).json({ error: 'Reporte no encontrado' });
    res.json({ message:'Respuesta del proveedor guardada', row:q.rows[0] });
  } catch (err) {
    console.error('Error guardando respuesta proveedor:', err.message);
    res.status(500).json({ error:'No se pudo guardar la respuesta del proveedor' });
  }
});
  ...
  const result = await pool.query(`
    WITH supplier_names AS (
      ...
      SELECT lower(trim(COALESCE(ma.provider_name,''))) AS skey,
             ...
             COUNT(*) FILTER (WHERE lower(COALESCE(ar.provider_outcome,''))='cuenta_reemplazo')::int AS provider_replacements,
             COUNT(*) FILTER (WHERE lower(COALESCE(ar.provider_outcome,''))='reembolso')::int AS provider_refunds,
             COUNT(*) FILTER (WHERE lower(COALESCE(ar.provider_outcome,''))='sin_solucion')::int AS provider_no_solution,
             COUNT(*) FILTER (WHERE lower(COALESCE(ar.provider_outcome,''))='otro')::int AS provider_other,
    ...
    SELECT sb.supplier_id,sb.name,
           ...
           COALESCE(f.provider_replacements,0)::int AS provider_replacements,
           COALESCE(f.provider_refunds,0)::int AS provider_refunds,
           COALESCE(f.provider_no_solution,0)::int AS provider_no_solution,
           COALESCE(f.provider_other,0)::int AS provider_other,
