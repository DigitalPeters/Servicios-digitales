(function(){
  ...
  function buildTracePreview(trace){
    ...
    return `<section class="master-trace-preview">
      ...
      ${sales.length?`<div class="master-trace-sales"><b>Ventas / entregas</b>${sales.slice(0,6).map(v=>`<div><span>Pedido #${esc(v.orden_id)} · ${esc(v.modalidad||'venta')} ${v.perfil?`· Perfil: ${esc(v.perfil)}`:''}</span><strong>${esc(v.comprador_nombre||v.comprador_email||'Usuario')}</strong><small>${esc(v.comprador_tipo||'vendedor')}${v.distribuidor_nombre?` · Distribuidor: ${esc(v.distribuidor_nombre)}`:''} · $${money(v.orden_amount)} · ${esc(fmtDate(v.orden_creada))}${v.pin?` · PIN ${esc(v.pin)}`:''}</small></div>`).join('')}</div>`:''}
      ${reports.length?`<div class="master-trace-reports"><b>Fallas y reemplazos</b>${reports.slice(0,6).map(r=>`<div><span>Reporte #${esc(r.id)} · ${esc(r.issue_type||'falla')}</span><strong>${esc(r.papel==='usada_como_reemplazo'?'Esta cuenta se usó como reemplazo':'Esta cuenta fue reportada')}</strong><small>${esc(r.status||'')} · ${esc(r.resolution_type||'')} · pedido #${esc(r.order_id||'-')}${r.provider_name_snapshot?` · Proveedor: ${esc(r.provider_name_snapshot)}`:''}</small>${r.provider_reported_at?`<small>📤 Proveedor reportado: ${esc(fmtDate(r.provider_reported_at))}${r.provider_responded_at?` · 📥 Respondió: ${esc(fmtDate(r.provider_responded_at))}`:''}</small>`:''}</div>`).join('')}</div>`:''}
    </section>`;
  }
