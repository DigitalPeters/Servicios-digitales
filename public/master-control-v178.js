(function(){
  ...
  async function loadProviderRanking(){
    ensureUI();const box=document.getElementById('mcProviderRanking');try{box.innerHTML='<div class="small-text">Calculando…</div>';const from=document.getElementById('mcProviderFrom').value,to=document.getElementById('mcProviderTo').value;const d=await api(`/api/admin/master/provider-performance?from=${from}&to=${to}`);supplierRows=d.providers||[];
      ...
      box.innerHTML=supplierRows.length?supplierRows.map((r,i)=>`<article class="mc-provider-card score-${r.score>=85?'great':r.score>=70?'good':r.score>=55?'mid':'risk'}"><div class="mc-provider-rank">#${i+1}</div><div class="mc-provider-top"><div><h4>${esc(r.name)}</h4><span>${esc(r.classification)} · ${r.score}/100</span></div><b>${r.score}</b></div><div class="mc-provider-metrics"><span><small>Invertido</small><b>$${money(r.invested)}</b></span><span><small>Utilidad período</small><b>$${money(r.profit)}</b></span><span><small>ROI operativo</small><b>${pct(r.roi_pct)}</b></span><span><small>Ventas</small><b>${r.sales}</b></span><span><small>Fallas</small><b>${r.failures} · ${pct(r.failure_rate)}</b></span><span><small>Resp. proveedor</small><b>${Number(r.avg_provider_response_hours||0)>0?Number(r.avg_provider_response_hours).toFixed(1)+' h':'—'}</b></span><span><small>Reportes medidos</small><b>${Number(r.provider_responses||0)}/${Number(r.provider_reports||0)}</b></span><span><small>≤24 h</small><b>${r.provider_responses>0?pct((Number(r.responses_under_24h||0)/Number(r.provider_responses||1))*100):'—'}</b></span><span><small>Pendientes proveedor</small><b>${Number(r.pending_provider_reports||0)}</b></span><span><small>Otra cuenta</small><b>${Number(r.provider_replacements||0)}</b></span><span><small>Reembolsos proveedor</small><b>${Number(r.provider_refunds||0)}</b></span><span><small>Sin solución</small><b>${Number(r.provider_no_solution||0)}</b></span><span><small>Reemplazos</small><b>${r.replacements} · ${pct(r.replacement_rate)}</b></span><span><small>Costo fallas</small><b>$${money(r.replacement_cost)}</b></span><span><small>Atención</small><b>${Number(r.avg_attention_rating||0).toFixed(1)}/5</b></span><span><small>Solución prom.</small><b>${Number(r.avg_resolution_hours||0)>0?Number(r.avg_resolution_hours).toFixed(1)+' h':'—'}</b></span></div><p>${esc(r.recommendation)}</p><div class="mc-score-parts"><em>Rentabilidad ${Math.round(r.profitability_score)}</em><em>Calidad ${Math.round(r.quality_score)}</em><em>Atención ${Math.round(r.attention_score)}</em><em>Resolución ${Math.round(r.resolution_score)}</em></div></article>`).join(''):'<div class="master-empty">Aún no hay proveedores con datos.</div>';
      fillSupplierOptions();
