  ...
  try{
    ...
    const data=await api('/api/admin/create-product',{
      ...
      body:JSON.stringify({
        ...
        renewable:document.getElementById('productRenewable')?.checked === true,
    ...
    if(document.getElementById('productRenewable'))document.getElementById('productRenewable').checked=false;
  ...
  const renderAdminProduct=(p)=>{
    ...
    return `<div class="item" id="admin-product-${p.id}" data-product-active="${isVisible?1:0}">
      ...
      <div class="admin-product-body">
        ...
        <div class="order-data" style="margin-top:12px"><label class="checkbox-row"><input type="checkbox" id="editRenewable-${p.id}" ${Number(p.renewable||0)===1?'checked':''} /> 🔄 Permitir renovación de este producto</label><div style="margin-top:8px"><label class="field-label">Días por renovación</label><input id="editRenewalDays-${p.id}" type="number" min="1" value="${Math.max(1,Number(p.renewal_days||30))}" /></div><p class="small-text">Los vendedores y distribuidores solo verán para renovar los productos habilitados.</p></div>
...
async function updateProduct(id){
  try{
    ...
    const data=await api('/api/admin/products/'+id,{method:'PATCH',body:JSON.stringify({
      ...
      renewable:document.getElementById(`editRenewable-${id}`)?.checked === true,
