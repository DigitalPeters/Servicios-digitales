  ...
  async function loadMasterOperations(showFeedback=false){
    ...
    try{
      ...
      if(kpis) kpis.innerHTML=`
        ...
        <button class="master-kpi master-kpi-profit" onclick="showSection('profit-quality')"><span class="master-kpi-top"><i>📈</i> Utilidad bruta</span><b>$${money(d.sales_today?.gross_profit)}</b><small>Ingreso real del admin menos costo real de compra</small></button>
        ...
        <button class="master-kpi ${Number(d.quarantine)>0?'master-kpi-warn':''}" onclick="openQuarantineFromDashboard(event)"><span class="master-kpi-top"><i>♻️</i> Cuarentena</span><b>${Number(d.quarantine||0)}</b><small>Cuentas por recuperar</small></button>
